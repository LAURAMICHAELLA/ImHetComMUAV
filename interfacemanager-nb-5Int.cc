...
uint32_t trafficSize;
uint32_t packetSizetraffic;
...
//uint32_t mbs = 0;
double totalPhyTxBytes;
double mbs, acum_mbs, acum_totalPhyTxBytes, acum_trafficSize;
...
std::string m_CSVfileNameReal = "RealThroughout.output.csv";
std::string m_CSVfileNameNN = "NeuralNetwork.output.csv";
...
std::ofstream outReal (m_CSVfileNameReal.c_str (), std::ios::app);
std::ofstream outNN (m_CSVfileNameNN.c_str (), std::ios::app);
...
WifiPhyStats::PhyTxTrace (std::string context, Ptr<const Packet> packet, WifiMode mode, WifiPreamble preamble, uint8_t txPower)
{
  ...
  trafficSize = pktSize;
  packetSizetraffic = pktSize;
  acum_trafficSize += trafficSize;


  m_phyTxBytes += pktSize;
  countDynamicRun++;
  NS_LOG_UNCOND ("Received PHY size=" << pktSize);
  std::cout << "Dynamic Traffic Size" << acum_trafficSize << std::endl;
...
void CalculatePhyRxDrop (Ptr<WifiPhyStats> m_wifiPhyStats)
 {
   ...
   DropBytes = totalPhyTxBytes2 - totalPhyRxDrop;
...
CheckThroughput (Ptr<WifiPhyStats> m_wifiPhyStats)
{
  ...
  totalPhyTxBytes = m_wifiPhyStats->GetTxBytes ();
  mbs = (m_bytesTotal * 8.0) / 1000000; // 1Mb
//  std::cout << "TESTING THROUGHPUT - REAL" << std::endl;
//  std::cout << "Dynamic Throughput" << "," << mbs << ", total Bytes Received, " << totalPhyTxBytes << std::endl;
  if (mbs != 0) {
      acum_mbs = acum_mbs + mbs;
      countMbs++;
    //  std::cout << "Acum Throughput" << "," << acum_mbs << ", Qtd samples, " << countMbs << std::endl;
  }
  acum_totalPhyTxBytes = acum_totalPhyTxBytes + totalPhyTxBytes;
...
double calc_prob(double x, double m, double s)
{
...
std::vector<std::vector<float>> Read_Iris_Dataset(void) {
  ...
  std::ifstream myfile("/home/doutorado/ns-allinone-3.30.1/ns-3.30.1/build/scratch/NaiveBayes/train.txt");
  std::string line;
  std::vector<std::vector<float>> Iris_Dataset;
  std::vector<float> temp_tam_packet;
  std::vector<float> temp_throughput;
  std::vector<float> temp_signal;
  std::vector<float> temp_SNR;
  std::vector<float> temp_high_weigth;


  float tam_packet_f, throughput_f, signal_f, SNR_f;
  float high_weigth_f;
  ...
  while (std::getline(myfile, line)) {
        ...
        iis >> tam_packet_f >> throughput_f >> signal_f >> SNR_f >> temp_string;

        temp_tam_packet.push_back(tam_packet_f);
        temp_throughput.push_back(throughput_f);
        temp_signal.push_back(signal_f);
        temp_SNR.push_back(SNR_f);

        if (temp_string.compare("short_packet") == 0)
        {

          high_weigth_f = short_packet;

        }
        else if (temp_string.compare("mediumsize_packet")==0){

          high_weigth_f = mediumsize_packet;

        }
        else if (temp_string.compare("large_packet")==0){

          high_weigth_f = large_packet;

        } else
        {

          high_weigth_f = unknown;
        }
        temp_high_weigth.push_back(high_weigth_f);
        }

        Iris_Dataset.push_back(temp_tam_packet);
        Iris_Dataset.push_back(temp_throughput);
        Iris_Dataset.push_back(temp_signal);
        Iris_Dataset.push_back(temp_SNR);
        Iris_Dataset.push_back(temp_high_weigth);
...
class_summary calculate_Class_Summary (std::vector<std::vector<float>> dataset, float class_label )
{
    ...
    {
        temp.clear();
        temp.push_back(Mean(*row));
        temp.push_back(Var(*row));
...
float NaiveBayes () {
      ...
      std::cout<< "-----Naive Bayes Runs------" <<std::endl;
    ...
    std::vector< std::vector<float>> temp_out4;

     std::vector<float> temp_tam_packet4;
     std::vector<float> temp_throughput4;
     std::vector<float> temp_signal4;
     std::vector<float> temp_SNR4;
     std::vector<float> temp_high_weigth4;


    // getting dynamic values
    /** Dynamic sensed metrics */
//    uint32_t pktSize = packet->GetSize ();  // first attribute
//throughputWifi [i] = mbs2;
//totalBytesReceivedSumWifi = throughputWifi [i] + totalBytesReceivedSumWifi;
//  Ptr<WifiPhyStats> m_wifiPhyStats;

  //double totalPhyTxBytes = m_wifiPhyStats->GetTxBytes ();
  //double mbs2 = (m_bytesTotal * 8.0) / 1000000; // 1Mb

    //double mbs = (m_bytesTotal * 8.0) / 1000000; // 1Mb
    // global variables g_signalDbmAvg, g_SNR  // third and fourth attribute

    //int j=0;
  //  float a1,b2,c3,d4,e;
  //  float sum_a, sum_b, sum_c, sum_d;

//    double acum_mbs, acum_totalPhyTxBytes; // throughput em 10 samples of 100ms which equals 1s of IM execution
    ...
    //  a=2001;
     ...
     std::cout<< "message size," << packetSizetraffic << "," << "throughput, " << mbs << ",totalBytes," << m_phyTxBytes<< ",signal," << g_signalDbmAvg << ",SNR," << g_SNR << std::endl;


    std::cout<< "transmissions," << countDynamicRun << " THROUGHPUT measurement" << countMbs << std::endl;

    a=acum_trafficSize/countDynamicRun; // divide by four because de interval of packet generate
    b=acum_mbs/countMbs;
    c = g_signalDbmAvg*(-1); // dont need to divide by four because itself realize the avg basing into amount of tx
    d = g_SNR;
    e=0;
  //  f=acum_totalPhyTxBytes/4;

    //std::cout<< "TEST VECTOR size: " << temp_tam_packet4[0] << "," << "TRAIN through class: " << temp_throughput4[0] << std::endl;
...
//    e=0;
    ...
    // human classify using size packet and thourghput in order to obtain score
    if ((a>= 5 && a<=15000) && (b>= 0 && b<=0.1))
    {

      e = short_packet;

    }
      else if ((a>1500 && a<=2000) && (b> 0.1 && b<=0.3)){

      e = mediumsize_packet;

    }
    else if ((a>2000) && (b> 0.3)){
    ...
    temp_tam_packet4.push_back(a);
    temp_throughput4.push_back(b);
    temp_signal4.push_back(c);
    temp_SNR4.push_back(d);
    temp_high_weigth4.push_back(e);
//    std::cout<< "TRAIN VECTOR size: " << temp_tam_packet4[0] << "," << "TRAIN through class: " << temp_throughput4[0] << std::endl;

    temp_out4.push_back(temp_tam_packet4);
    temp_out4.push_back(temp_throughput4);
    temp_out4.push_back(temp_signal4);
    temp_out4.push_back(temp_SNR4);
    temp_out4.push_back(temp_high_weigth4);

    std::cout << "TESTING - NB" << std::endl;
    // std::cout << "Dynamic Throughput" << "," << b << ", total Bytes Received, " << f << std::endl;

  //  std::cout<< "Used by test Naive Bayes" << std::endl;
    std::cout<< "message size," << a << "," << "throughput, " << b << ",signal," << c << ",SNR," << d << std::endl;



    // std::vector<std::vector<float>> testing_data =dynamic_Test_Split();
     std::cout<< "IRIS testing Data Size is ( " << temp_out4.size() << " , "  <<temp_out4[0].size()<<" )" <<std::endl;


    std::vector<std::vector<float>> training_data =  vector_Train_Split(dataset , 100);
    std::cout<< "IRIS training Data Size is ( " << training_data.size() << " , "  <<training_data[0].size()<<" )" <<std::endl;
    naive.fit(training_data);


    std::vector<float> temp_tam_packet;
    std::vector<float> temp_class;
    std::vector<float> temp_tam_packet2;
    std::vector<float> temp_throughput;
    std::vector<float> temp_signal;
    std::vector<float> temp_SNR;
    std::vector<float> temp_class2;

    std::vector< std::vector<float> > temp_out;
    std::vector< std::vector<float> > out_Vector;
    int counter=0;

    std::cout<< "<<<<<<<<Running Data of Training>>>>>>>>>>" <<std::endl;


    std::for_each( training_data[0].begin(), training_data[0].end(), [&](float & item_class)
    {
          std::vector<float> temp;
          temp_tam_packet.push_back(training_data[0][counter]);
          temp_class.push_back(training_data[4][counter]);
          //std::cout<< "size packet, " << training_data[0][counter]<< ", throughput," << training_data[1][counter]<< ", RSSI," << training_data[2][counter] << ", SNR," << training_data[3][counter] << ",class, " << training_data[4][counter]<< std::endl;
          //temp_out.push_back(temp);
      ...
      // std::vector<float> temp;
      temp_tam_packet2.push_back(temp_out4[0][0]);
      temp_throughput.push_back(temp_out4[1][0]);
      temp_signal.push_back(temp_out4[2][0]);
      temp_SNR.push_back(temp_out4[3][0]);
      temp_class2.push_back(temp_out4[4][0]);


      std::cout<< "size packet, " << temp_out4[0][0]<< "," <<  "throughput, " << temp_out4[1][0]<< ", signal, " << temp_out4[2][0]<< ", SNR, " << temp_out4[3][0]<< ", class, " << temp_out4[4][0]<< std::endl;
  ...
  //		std::cout<< "total " << temp<< std::endl;
      ...
      float current_predict;
      std::vector<float> predicitions;
      temp_out4 = vect_Transpose(temp_out4);
      for(size_t i=0; i<temp_out4.size(); i++ )
      {
          auto index = naive.predict(temp_out4[i]);
          predicitions.push_back(index);
          std::cout<< "predictions, " << predicitions[i] << std::endl;
          current_predict = predicitions[i];
      }
      temp_out4 = vect_Transpose(temp_out4);
...
//std::cout<<"score is :" << Vect_match_Score (temp_out4[4],predicitions)<<std::endl;
      ...
      std::cout<<"Naive Bayes time of END execution :" << milliseconds<<std::endl;


      char Str[100];


      FILE * gp = fopen("Naive.txt", "wt");

      strcpy(Str,"(size, throughput, signal, SNR, Human class)-----ML prediction, Score ");
      fprintf(gp, "%s\n",Str);
      fprintf(gp, "%f %f %f %f %f ------- %f %f \n", temp_out4[0][0], temp_out4[1][0], temp_out4[2][0], temp_out4[3][0], temp_out4[4][0], predicitions[0], Vect_match_Score (temp_out4[4],predicitions));
...
void InterfaceManager(NodeContainer ueNode, UdpClientHelper dlClient, UdpClientHelper wdlClient, UdpClientHelper acClient, UdpClientHelper ax2_4Client, UdpClientHelper ax5Client){
    ...
    float nb;
    nb=NaiveBayes();
    std::cout << "<<<<<<<<<<<<< Starting IM with Naive Bayes classifier>>>>>>>>>>>" << nb << std::endl;
    std::cout << "now: " << Simulator::Now ().GetSeconds () << " s" << std::endl;
    std::cout << "------ Naive Bayes defined weight politic>>>>>>" << nb << std::endl;
    ...
    std::cout << "vector of weight: " << w1 << "," << w2 << "," << w3 << std::endl;
...
if (numberofexecution > 3) {
                ...
                //if (Simulator::Now ().GetSeconds () > 5000000000) {
                      ...
                      // bytes throughput --- c3  --- politic of weight
                      if (AvgthroughputWave > AvgthroughputWifi && AvgthroughputWave > AvgthroughputAc && AvgthroughputWave > AvgthroughputAx_2_4 && AvgthroughputWave > AvgthroughputAx5){
                                sumPointsIntB = (sumPointsIntB + 1)* w1;
                                std::cout << "sumPointsIntB:" << sumPointsIntB << std::endl;
                                        ...
                                        sumPointsIntA = (sumPointsIntA + 1)* w1;
                                        std::cout << "sumPointsIntA: " << sumPointsIntA << std::endl;
                                                  ...
                                                  sumPointsIntC = (sumPointsIntC + 1) * w1;
                                                  std::cout << "sumPointsIntC: " << sumPointsIntC << std::endl;
                                        } else if (AvgVetBytesDropAx_2_4 > AvgVetBytesDropWifi && AvgVetBytesDropAx_2_4 > AvgVetBytesDropWave && AvgVetBytesDropAx_2_4 > AvgVetBytesDropAc && AvgVetBytesDropAx_2_4 > AvgVetBytesDropAx5) {
                                                      sumPointsIntD = (sumPointsIntD + 1) * w1;
                                                      std::cout << "sumPointsIntD: " << sumPointsIntD << std::endl;
                                                    } else if (AvgVetBytesDropAx5 > AvgVetBytesDropWifi && AvgVetBytesDropAx5 > AvgVetBytesDropWave && AvgVetBytesDropAx5 > AvgVetBytesDropAc && AvgVetBytesDropAx5 > AvgVetBytesDropAx_2_4) {
                                                                  sumPointsIntE = (sumPointsIntE + 1) * w1;
                                                                  std::cout << "sumPointsIntE: " << sumPointsIntE << std::endl;
                                                                  }
                      ...
                      //signal received Dbm --- c5  -- politic of weight
                      if (AvgVet_g_signalDbmAvgWave > AvgVet_g_signalDbmAvgWifi && AvgVet_g_signalDbmAvgWave > AvgVet_g_signalDbmAvgAc && AvgVet_g_signalDbmAvgWave > AvgVet_g_signalDbmAvgAx_2_4 && AvgVet_g_signalDbmAvgWave > AvgVet_g_signalDbmAvgAx5){
                                sumPointsIntB = (sumPointsIntB + 1) * w2;
                                std::cout << "sumPointsIntB: " << sumPointsIntB << std::endl;
                                        ...
                                        sumPointsIntA = (sumPointsIntA + 1) * w2;
                                        std::cout << "sumPointsIntA: " << sumPointsIntA << std::endl;
                                                  ...
                                                  sumPointsIntC = (sumPointsIntC + 1) * w2;
                                                  std::cout << "sumPointsIntC: " << sumPointsIntC << std::endl;
                                                      ...
                                                      sumPointsIntD = (sumPointsIntD + 1) * w2;
                                                      std::cout << "sumPointsIntD: " << sumPointsIntD << std::endl;
                                                                  ...
                                                                  sumPointsIntE = (sumPointsIntE + 1) * w2;
                                                                  std::cout << "sumPointsIntE: " << sumPointsIntE << std::endl;
                                                                  }
                      ...
                      if (AvgVet_g_SNRWave > AvgVet_g_SNRWifi && AvgVet_g_SNRWave > AvgVet_g_SNRAc && AvgVet_g_SNRWave > AvgVet_g_SNRAx_2_4 && AvgVet_g_SNRWave > AvgVet_g_SNRAx5){
                                sumPointsIntB = (sumPointsIntB + 1) * w3;
                                std::cout << "sumPointsIntB: " << sumPointsIntB << std::endl;
                                        ...
                                        sumPointsIntA = (sumPointsIntA + 1) * w3;
                                        std::cout << "sumPointsIntA: " << sumPointsIntA << std::endl;
                                                  ...
                                                  sumPointsIntC = (sumPointsIntC + 1) * w3;
                                                  std::cout << "sumPointsIntC: " << sumPointsIntC << std::endl;
                                                      ...
                                                      sumPointsIntD = (sumPointsIntD + 1) * w3;
                                                      std::cout << "sumPointsIntD: " << sumPointsIntD << std::endl;
                                                                  ...
                                                                  sumPointsIntE = (sumPointsIntE + 1) * w3;
                                                                  std::cout << "sumPointsIntE: " << sumPointsIntE << std::endl;
                                                                  }
