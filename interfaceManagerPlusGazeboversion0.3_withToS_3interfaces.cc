...
void vetorBytesReceived (std::string context, Ptr <const Packet> p)
{
  ...
  if (iph.GetDestination()=="192.168.1.1" || iph.GetDestination()=="192.168.1.2" || iph.GetDestination()=="192.168.1.3" ||
    ...
    if (iph.GetDestination()=="20.1.1.1" || iph.GetDestination()=="20.1.1.2" || iph.GetDestination()=="20.1.1.3" ||
       iph.GetDestination()=="20.1.1.4" || iph.GetDestination()=="20.1.1.5" || iph.GetDestination()=="20.1.1.6" ||
       iph.GetDestination()=="20.1.1.7" || iph.GetDestination()=="20.1.1.8" || iph.GetDestination()=="20.1.1.9" || iph.GetDestination()=="20.1.1.10") {
...
//InterMan
void InterfaceManager(NodeContainer ueNode, UdpClientHelper dlClient, UdpClientHelper wdlClient, UdpClientHelper acClient){
    //sleep (5);
    int sumPointsIntA=0, sumPointsIntB=0, sumPointsIntC=0;// c=0; // A=Wifi 2.4GHz B = Wave 5.9GHz C=802.11ac D=802.11ax 2.4GHz E=802.11ax 5GHz
    int change;
...
if (numberofexecution > 3) {
    ...
    Simulator::Schedule (Seconds (1), &InterfaceManager, ueNode, dlClient, wdlClient, acClient);
  }
...
//          // Only set the channel width and guard interval for HT and VHT modes
   ...
   std::cout << "Wifi_2.4--Wave5--Ac Intefaces Installed!. Done!" << std::endl;
...
//
        ...
        //InterMan
         InterfaceManager (ueNode, dlClient, wdlClient, acClient);
