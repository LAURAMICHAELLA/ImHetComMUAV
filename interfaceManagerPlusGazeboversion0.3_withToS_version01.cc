   ...
   std::cout << "Wifi+Wave Intefaces Installed!. Done!" << std::endl;
    ...
    for (int i = 0; i < numberOfUEs; i++)
      ...
      //    void CheckThroughput ();
          ...
          UdpServerHelper wdlPacketSinkHelper (wdport);
          UdpServerHelper acPacketSinkHelper (acport);
                ...
                for (uint8_t tosValue : tosValues)
                  {
                    ...
                    for (uint32_t u = 0; u < ueNode.GetN (); ++u)
                    {
                      ...
                      sinkSocket.SetTos (tosValue); // for interface A
                      sinkSocket2.SetTos (tosValue); // for interface B
                      sinkSocket3.SetTos (tosValue); // for interface C

                      OnOffHelper onOffHelper ("ns3::UdpSocketFactory", sinkSocket);
                      OnOffHelper onOffHelper1 ("ns3::UdpSocketFactory", sinkSocket2);
                      OnOffHelper onOffHelper2 ("ns3::UdpSocketFactory", sinkSocket3);
                      ...
                      onOffHelper2.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1.0]"));
                      ...
                      onOffHelper.SetAttribute ("DataRate", DataRateValue (50000000 / numberOfUEs)); //50 Mb/s
                      onOffHelper1.SetAttribute ("DataRate", DataRateValue (50000000 / numberOfUEs)); //50 Mb/s
                      onOffHelper2.SetAttribute ("DataRate", DataRateValue (50000000 / numberOfUEs)); //50 Mb/s

                      onOffHelper.SetAttribute ("PacketSize", UintegerValue (pktSize)); //bytes
                      onOffHelper1.SetAttribute ("PacketSize", UintegerValue (pktSize)); //bytes
                     ...
                     // sinkApplications.Add (packetSinkHelper2.Install (ueNode.Get (u)));
                     // sinkApplications.Add (packetSinkHelper3.Install (ueNode.Get (u)));
                     //
                     // sourceApplications.Add (onOffHelper.Install (ueNode.Get (u)));
                     // sourceApplications.Add (onOffHelper1.Install (ueNode.Get (u)));
                     // sourceApplications.Add (onOffHelper2.Install (ueNode.Get (u)));

                     sinkApplications.Add (dlClient.Install (ueNode.Get(u)));
                     sourceApplications.Add (dlPacketSinkHelper.Install (ueNode.Get(u)));

                     sinkApplications.Add (wdlClient.Install (ueNode.Get(u)));
                     sourceApplications.Add (wdlPacketSinkHelper.Install (ueNode.Get(u)));

                     sinkApplications.Add (acClient.Install (ueNode.Get(u)));
                     sourceApplications.Add (acPacketSinkHelper.Install (ueNode.Get(u)));
...
//*          ApplicationContainer clientApps, serverApps;
    //*      ApplicationContainer sourceApplications, sinkApplications;
      ...
      //     for (uint32_t i=0; i < ueNode.GetN ();i++){
      //                   sinkApplications.Add (dlClient.Install (ueNode.Get(i)));
      //           }
      //
      //
      //
      ...
      //       }
      //
      //       PacketSinkHelper acPacketSinkHelper ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), acport));
      //
      //       for (uint32_t i=0; i < ueNode.GetN ();i++){
...
//        std::cout << "USING Timer Seconds!--->" <<s.GetSeconds() << "s" << std::endl;
        ...
        //InterMan
        // the InterMan will run always up to three interfaces, but we can run doing combinations of three

        // switch (NumberOfInterfaces) {
        //   case 2: // 802.11.n and 802.11p
        //   InterfaceManager (ueNode, dlClient, wdlClient, dport, wdport);
        //
        //
        //   case 3: // 802.11n, 802.11p, 802.11ac
        //   InterfaceManager (ueNode, dlClient, wdlClient, dport, wdport);
        //   InterfaceManager (ueNode, dlClient, acClient, dport, acport);
        //   InterfaceManager (ueNode, acClient, wdlClient, dport, wdport);
        //
        //   case 4: // 802.11n, 802.11p, 802.11ac, 802.11ax
        //   InterfaceManager (ueNode, dlClient, wdlClient, dport, wdport);
        //   InterfaceManager (ueNode, acClient, wdlClient, dport, wdport);
        //   InterfaceManager (ueNode, acClient, wdlClient, dport, wdport);
        //
        //   case 5: // 802.11n, 802.11p, 802.11ac, 802.11ax, LTE
        // }
