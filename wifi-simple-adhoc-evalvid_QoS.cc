...
NS_LOG_COMPONENT_DEFINE ("WifiSimpleAdhoc_EDCA");
...
int main (int argc, char *argv[])
{
  ...
  std::string m_traceFile= "/home/doutorado/sumo/examples/fanet_old/different_speed_sumo/mobility_manyspeed.tcl";
  std::string m_lossModelName;

  uint32_t m_mobility = 1; ///< mobility
  uint32_t m_nNodes = 3; ///< number of nodes
  double m_TotalSimTime = 90.0;        ///< seconds
...
//  wifiChannel.AddPropagationLoss ("ns3::FixedRssLossModel","Rss",DoubleValue (rss));
  ...
  // Add a mac and set rate control
    // Setting the EDCA Enhanced Distributed Channel Access
  WifiMacHelper wifiMac;

  // this will configure the MAC layer to be wireless and support QosWifiMacHelper
...
Ptr<NetDevice> dev = c.Get(1)-> GetDevice(0);
...
edca->SetTxopLimit (MicroSeconds(3008));
  ...
  ApplicationContainer serverApp = serverA.Install (c.Get(1));
  serverApp.Start (Seconds (1.0));
  serverApp.Stop (Seconds (m_TotalSimTime + 1));

  InetSocketAddress destA (i.GetAddress (0), port);
  ...
  clientA.SetAttribute ("PacketSize", UintegerValue (packetSize));

  ApplicationContainer clientAppA = clientA.Install (c.Get (0));
  clientAppA.Start (Seconds (2.0));
  clientAppA.Stop (Seconds (m_TotalSimTime + 1));
...
*/
  ...
  /* Show results */
  uint64_t totalPacketsThroughA = DynamicCast<UdpServer> (serverApp.Get (1))->GetReceived ();

  NS_LOG_INFO ("Run Simulation.");
  Simulator::Stop (Seconds (m_TotalSimTime + 1));
  Simulator::Run ();
  Simulator::Destroy ();

 double throughput = totalPacketsThroughA * packetSize * 8 / (m_TotalSimTime * 1000000.0);
  std::cout << "Throughput for AC_VI with default TXOP limit (3008ms): " << throughput << " Mbit/s" << '\n';
