...
uint16_t port = 1; // WiFi
uint16_t port2 = 2; // Wave
uint16_t port3 = 3; // Ac
uint16_t port4 = 4; // Ax 2.4
uint16_t port5 = 5; // Ax 5.0
                ...
                for (uint8_t tosValue : tosValues)
                  {
                          ...
                          for (uint32_t u = 0; u < ueNode.GetN (); ++u)
                          {
                            ...
                            if (Simulator::Now ().GetSeconds () > 1 && Simulator::Now ().GetSeconds () < duration/2) {
                            ...
                            if (Simulator::Now ().GetSeconds () > duration/2) {
                              ...
                              EvalvidServerHelper server (portNumber);
                              ...
                              EvalvidClientHelper client (interface.GetAddress (1,0),portNumber);
                              client.SetAttribute ("ReceiverDumpFilename", StringValue(novoId));
                              apps = client.Install (ueNode.Get (u));

                              std::cout << "Now:" << Simulator::Now ().GetSeconds () << ",Sending video streamming from "<< ueNode.Get(u) << "---port--- " << portNumber << ",Tos=" << "AC_VI" << ",Addr=" << addr << ";" << std::endl;
                  ...
                  for (uint8_t tosValue : tosValues)
                    {
                      for (uint32_t u = 0; u < ueNode.GetN (); ++u)
                      {
                         ...
                         if (Simulator::Now ().GetSeconds () > duration/2) {
                           ...
                           EvalvidServerHelper server (portNumber2);
                           server.SetAttribute ("SenderTraceFilename", StringValue("st_highway_cif.st"));
                           ...
                           EvalvidClientHelper client (interface.GetAddress (2,0),portNumber2);
                           client.SetAttribute ("ReceiverDumpFilename", StringValue(novoId_2));
                           apps = client.Install (ueNode.Get (u));

                           std::cout << "Now:" << Simulator::Now ().GetSeconds () << ",Sending video streamming from "<< ueNode.Get(u) << "---port--- " << portNumber2 << ",Tos=" << "AC_VI" << ",Addr=" << address2 << ";" << std::endl;
                  ...
                  for (uint32_t u = 0; u < ueNode.GetN (); ++u)
                  {
                    ...
                    if (Simulator::Now ().GetSeconds () > duration/2) {
                                ...
                                char novoId_3[10];
                                  ...
                                  EvalvidServerHelper server (portNumber3);
                                  server.SetAttribute ("SenderTraceFilename", StringValue("st_highway_cif.st"));
                                  ...
                                  EvalvidClientHelper client (interface.GetAddress (3,0),portNumber3);
                                  client.SetAttribute ("ReceiverDumpFilename", StringValue(novoId_3));
                                  apps = client.Install (ueNode.Get (u));

                                  std::cout << "Now:" << Simulator::Now ().GetSeconds () << ",Sending video streamming from "<< ueNode.Get(u) << "---port--- " << portNumber3 << ",Tos=" << "AC_VI" << ",Addr=" << address3 << ";" << std::endl;
            ...
            for (uint8_t tosValue : tosValues)
              {
                    ...
                    if (Simulator::Now ().GetSeconds () > duration/2) {
                            ...
                            char novoId_4[10];
                              ...
                              EvalvidServerHelper server (portNumber4);
                              server.SetAttribute ("SenderTraceFilename", StringValue("st_highway_cif.st"));
                              ...
                              EvalvidClientHelper client (interface.GetAddress (4,0),portNumber4);
                              ...
                              apps.Stop (Seconds (duration+1.0));
                                ...
                                std::cout << "Now:" << Simulator::Now ().GetSeconds () << ",Sending video streamming from "<< ueNode.Get(u) << "---port--- " << portNumber4 << ",Tos=" << "AC_VI" << ",Addr=" << address4 << ";" << std::endl;
                                i_4++;
                    ...
                    for (uint8_t tosValue : tosValues)
                      {
                            ...
                            if (Simulator::Now ().GetSeconds () > duration/2) {
                                          ...
                                          char novoId_5[10];
                                            ...
                                            EvalvidServerHelper server (portNumber5);
                                            server.SetAttribute ("SenderTraceFilename", StringValue("st_highway_cif.st"));
                                            ...
                                            EvalvidClientHelper client (interface.GetAddress (5,0),portNumber5);
                                            ...
                                            std::cout << "Now:" << Simulator::Now ().GetSeconds () << ",Sending video streamming from "<< ueNode.Get(u) << "---port--- " << portNumber5 << ",Tos=" << "AC_VI" << ",Addr=" << address5 << ";" << std::endl;
                                            i_5++;
