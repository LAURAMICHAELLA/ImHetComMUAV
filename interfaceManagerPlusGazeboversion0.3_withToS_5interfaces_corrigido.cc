   ...
   std::cout << "Wifi_2.4--Wave5--Ac5--Ax2.4---Ax5 Intefaces Installed!. Done!" << std::endl;
    ...
    for (int i = 0; i < numberOfUEs; i++)
      ...
      //    void CheckThroughput ();
          ...
          uint32_t portNumber = 100;
            ...
            //  {
                for (uint8_t tosValue : tosValues)
                  ...
                  sinkApplications.Start (Seconds (5));
                  sinkApplications.Stop (Seconds (duration/2));
                  sourceApplications.Start (Seconds (5.5));



                  if (Simulator::Now ().GetSeconds () > duration/2) {
                    ...
                    char novoId[10];
                                    ...
                                    for (uint32_t u = 0; u < ueNode.GetN (); ++u)
                                    {
                                      ...
                                      InetSocketAddress evalvid_intA (address, port++);

                                      evalvid_intA.SetTos (0xb8); //AC_VI
                                      EvalvidServerHelper server (port);
                                      server.SetAttribute ("SenderTraceFilename", StringValue("st_highway_cif.st"));
                                      server.SetAttribute ("SenderDumpFilename", StringValue("sd_a"));
                                      ...
                                      std::cout << "Now:" << Simulator::Now ().GetSeconds () << ",Sending video streamming from "<< ueNode.Get(u) << "---port--- " << port << ",Tos=" << "AC_VI" << ",Addr=" << addr << ";" << std::endl;
          ...
          sinkApplications2.Start (Seconds (5.1));
          sinkApplications2.Stop (Seconds (duration/2));
          sourceApplications2.Start (Seconds (5.6));
          ...
          if (Simulator::Now ().GetSeconds () > duration/2) {
                    ...
                    for (uint32_t u = 0; u < ueNode.GetN (); ++u)
                    {
                      ...
                      InetSocketAddress evalvid_intB(address2, port++);

                      evalvid_intB.SetTos (0xb8); //AC_VI
                      EvalvidServerHelper server (port);
                      server.SetAttribute ("SenderTraceFilename", StringValue("st_highway_cif.st"));
                      server.SetAttribute ("SenderDumpFilename", StringValue("sd_b"));
                      server.SetAttribute ("PacketPayload",UintegerValue(pktSize));
                      ...
                      snprintf (novoId_2, 10, "%s%d", id_2, i_2);
                      ...
                      std::cout << "Now:" << Simulator::Now ().GetSeconds () << ",Sending video streamming from "<< ueNode.Get(u) << "---port--- " << port << ",Tos=" << "AC_VI" << ",Addr=" << address2 << ";" << std::endl;
      ...
      sinkApplications3.Start (Seconds (5.2));
      sinkApplications3.Stop (Seconds (duration/2));
      sourceApplications3.Start (Seconds (5.3));
      ...
      if (Simulator::Now ().GetSeconds () > duration/2) {
                  ...
                  for (uint32_t u = 0; u < ueNode.GetN (); ++u)
                  {
                    ...
                    InetSocketAddress evalvid_intC (address3, port++);

                    evalvid_intC.SetTos (0xb8); //AC_VI
                    EvalvidServerHelper server (port);
                    server.SetAttribute ("SenderTraceFilename", StringValue("st_highway_cif.st"));
                    server.SetAttribute ("SenderDumpFilename", StringValue("sd_c"));
                    server.SetAttribute ("PacketPayload",UintegerValue(pktSize));
                    ...
                    snprintf (novoId_3, 10, "%s%d", id_3, i_3);
                    ...
                    std::cout << "Now:" << Simulator::Now ().GetSeconds () << ",Sending video streamming from "<< ueNode.Get(u) << "---port--- " << port << ",Tos=" << "AC_VI" << ",Addr=" << address3 << ";" << std::endl;
      ...
      uint32_t portNumber4 = 400;
        ...
        //  {
            ...
            for (uint8_t tosValue : tosValues)
              ...
              sinkApplications4.Start (Seconds (5.4));
              sinkApplications4.Stop (Seconds (duration/2));
              sourceApplications4.Start (Seconds (5.5));
      ...
      if (Simulator::Now ().GetSeconds () > duration/2) {
              ...
              for (uint32_t u = 0; u < ueNode.GetN (); ++u)
              {
                ...
                InetSocketAddress evalvid_intD (address4, port++);


                evalvid_intD.SetTos (0xb8); //AC_VI
                EvalvidServerHelper server (port);
                server.SetAttribute ("SenderTraceFilename", StringValue("st_highway_cif.st"));
                server.SetAttribute ("SenderDumpFilename", StringValue("sd_d"));
                server.SetAttribute ("PacketPayload",UintegerValue(pktSize));
                ...
                snprintf (novoId_4, 10, "%s%d", id_4, i_4);
                ...
                apps.Stop (Seconds (duration+1.0));
                  ...
                  std::cout << "Now:" << Simulator::Now ().GetSeconds () << ",Sending video streamming from "<< ueNode.Get(u) << "---port--- " << port << ",Tos=" << "AC_VI" << ",Addr=" << address4 << ";" << std::endl;
                  i_4++;
              ...
              uint32_t portNumber5 = 500;
                ...
                //  {
                    ...
                    for (uint8_t tosValue : tosValues)
                      ...
                      sinkApplications5.Start (Seconds (5.5));
                      sinkApplications5.Stop (Seconds (duration/2));
                      sourceApplications5.Start (Seconds (5.6));
      ...
      if (Simulator::Now ().GetSeconds () > duration/2) {
                    ...
                    for (uint32_t u = 0; u < ueNode.GetN (); ++u)
                    {
                      ...
                      InetSocketAddress evalvid_intE (address5, port++);

                      evalvid_intE.SetTos (0xb8); //AC_VI
                      EvalvidServerHelper server (port);
                      server.SetAttribute ("SenderTraceFilename", StringValue("st_highway_cif.st"));
                      server.SetAttribute ("SenderDumpFilename", StringValue("sd_e"));
                      server.SetAttribute ("PacketPayload",UintegerValue(pktSize));
                      ...
                      snprintf (novoId_5, 10, "%s%d", id_5, i_5);


                      EvalvidClientHelper client (interface.GetAddress (5,0),port);
                      client.SetAttribute ("ReceiverDumpFilename", StringValue("rd_a05"));
                      apps = client.Install (ueNode.Get (u));

                      apps.Start (Seconds (duration/2+0.5));
                      apps.Stop (Seconds (duration+1.0));

                      std::cout << "Now:" << Simulator::Now ().GetSeconds () << ",Sending video streamming from "<< ueNode.Get(u) << "---port--- " << port << ",Tos=" << "AC_VI" << ",Addr=" << address5 << ";" << std::endl;
                      i_5++;
