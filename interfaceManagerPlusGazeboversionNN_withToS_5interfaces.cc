...
void InterfaceManagerNN()
{
    ...
    vector<pair<double, int>> trainSet; //what means a good throughput in this scenario with this interfaces?
  //  vector<pair<double, int>> trainSetSignal; // whats means a good signal reception in this scenario with this interfaces?

    trainSet.resize(Train_Set_Size);
  //  trainSetSignal.resize(Train_Set_Size);

    // interfaces std::map<key, value> map;
    // key = 1, wave
    // key = 2, wifi
    // key = 3, ac
    // key = 4, ax2_4
    ...
    for (int i = 0; i < Train_Set_Size; i++) {
        ...
        if (AvgthroughputWave > AvgthroughputWifi && AvgthroughputWave > AvgthroughputAc && AvgthroughputWave > AvgthroughputAx_2_4 && AvgthroughputWave > AvgthroughputAx5){
                trainSet[i] = make_pair(AvgthroughputWave, 1);
                          ...
                          trainSet[i] = make_pair(AvgthroughputWifi, 2);
                                    ...
                                    trainSet[i] = make_pair(AvgthroughputAc, 3);
                                    ...
                                    trainSet[i] = make_pair(AvgVetBytesDropAx_2_4, 4);
                                              ...
                                              trainSet[i] = make_pair(AvgVetBytesDropAx5, 5);
                                                    }
    ...
    //                     trainSetSignal[i] = make_pair(AvgVet_g_signalDbmAvgWave, 1);
    ...
    //                             trainSetSignal[i] = make_pair(AvgVet_g_signalDbmAvgWifi, 2);
    ...
    //                                 trainSetSignal[i] = make_pair(AvgVet_g_signalDbmAvgAc, 3);
    ...
    //                                       trainSetSignal[i] = make_pair(AvgVet_g_signalDbmAvgAx_2_4, 4);
    ...
    //                                           trainSetSignal[i] = make_pair(AvgVet_g_signalDbmAvgAx5, 4);
    //                                                       }
    //
    //     }
    ...
    for (int j = 0; j < epoch; j++) {
      ...
      // for (int i = 0; i < Train_Set_Size; i++) {
      //   train(trainSetSignal[i].first, trainSetSignal[i].second);
      // }
      // std::cout << j << "\r";
  ...
  //  Plot the results
    ...
    for (int i = 0; i < 1000; i++) {
          x.push_back(trainSet[i].second);
          y1.push_back(trainSet[i].first);
          y2.push_back(f_theta(trainSet[i].first));
          outReal << Simulator::Now ().GetSeconds () << "," << y1[i] << "," << x[i] << std::endl;
          outNN << Simulator::Now ().GetSeconds () << "," << y2[i] << "," << x[i]  << std::endl;
    ...
    //   outReal << Simulator::Now ().GetSeconds () << "," << x[k] << "," << y1[k] << std::endl;
    //
    // }
    // //Neural Network Approximate f(x) = sin(x) -> Red Graph
    // for (int k = 0; k < 1000; k++) {
    //   outNN << Simulator::Now ().GetSeconds () << "," << x[k] << "," << y2[k] << std::endl;
    //
    // }

    outReal.close();
    outNN.close();
    Simulator::Schedule (Seconds (0.1), &InterfaceManagerNN);
...
//                            snprintf (novoId_sd_d, 20, "send:%d-%s-node:%d", run_ev, id_sdD, i_4);
  ...
  //      Config::Connect("/NodeList/*/$ns3::Node/ApplicationList/*/$ns3::PacketSocketServer/Rx", MakeCallback(&traceqos));
      ...
      //    CalculatePhyRxDrop (m_wifiPhyStats);
       ...
       Simulator::Schedule(Seconds(0.1), &CalculatePhyRxDrop, m_wifiPhyStats);
       Simulator::Schedule(Seconds(0.1), &CalculateAvgBuffer);
       Simulator::Schedule(Seconds(3), &InterfaceManagerNN);
